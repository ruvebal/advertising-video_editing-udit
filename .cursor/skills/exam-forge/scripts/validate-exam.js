 ...
 *   - MCQ first-option bias (correct answer is first position >30%)
...
function isWellFormedXhtml(html) {
	...
	const openTags = (trimmed.match(/<([a-zA-Z][a-zA-Z0-9]*)\b[^>]*(?<!\/)>/g) || []).map((t) =>
		t.match(/<([a-zA-Z][a-zA-Z0-9]*)/)[1].toLowerCase()
	);
	const closeTags = (trimmed.match(/<\/([a-zA-Z][a-zA-Z0-9]*)>/g) || []).map((t) =>
		t.match(/<\/([a-zA-Z][a-zA-Z0-9]*)/)[1].toLowerCase()
	);
...
function validate(inputPath) {
	...
	for (let i = 0; i < exam.questions.length; i++) {
		...
		if (q.type === 'multichoice' && q.answers && q.answers.length >= 2) {
			...
			for (const ca of correctAnswers) {
				...
				if (correctLen === maxLen && correctLen > 0) {
					...
					if (correctLen > secondLongest * 1.15) {
						longestBiasCount++;
						warnings.push(
							`${label}: correct answer is longest by >15% (${correctLen} vs ${secondLongest} chars) — longest-answer bias`
						);
					}
					break;
				}
			...
			// First-option bias check: correct answer is first position (index 0)
			const firstAnswer = q.answers[0];
			if (firstAnswer && Number(firstAnswer.fraction) > 0) {
				firstOptionBiasCount++;
			}
		}
	...
	if (totalObjective > 0) {
		...
		if (mcqCount > 0) {
			const firstOptionPct = (firstOptionBiasCount / mcqCount) * 100;
			if (firstOptionPct > 30) {
				warnings.push(
					`First-option bias: ${firstOptionBiasCount}/${mcqCount} MCQs (${firstOptionPct.toFixed(0)}%) have correct answer in position A — target ≤30%`
				);
			}
		}
	}
